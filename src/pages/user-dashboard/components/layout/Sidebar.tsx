...
// 1. Define the props interface for the main Sidebar component
...
// 2. Define the props interface for the helper SidebarItem component
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  // Helper component typed with SidebarItemProps
  const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, tabId }) => (
    <button
      onClick={() => { setActiveTab(tabId); closeMobile(); }}
      className={`group relative w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition-all duration-200 font-medium ${
        activeTab === tabId
      ...
      >
        <div className="p-3 h-full flex flex-col no-scrollbar overflow-y-auto relative">
          ...
          <div className="space-y-8 flex-1">
            ...
            <div>
              <p className={` font-bold text-gray-400 mb-4 px-4 transition-opacity duration-300 ${!isDesktopOpen && !isMobileOpen && 'md:hidden'}`}>OTHERS</p>
              <div className="space-y-2">
          ...
          <div className="mt-auto border-t border-gray-100 pt-6">
            <button onClick={() => alert("Logging out...")} className="group relative w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-red-500 hover:bg-red-50 hover:shadow-sm">
